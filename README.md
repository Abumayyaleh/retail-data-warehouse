...
1. Add safe parsing and rejected-row tables: malformed numeric IDs, numeric strings, or invalid dates can currently fail a refresh.
2. Make deduplication deterministic with source timestamps and stable tie-breakers; retain rejected-row reasons and source lineage.
3. Replace full rebuilds with incremental loads where justified and add slowly changing dimensions for historical attributes and costs.
4. Track missing-price/cost flags, validate discount null semantics, and define return, cancellation, tax, currency, and revenue policies.
